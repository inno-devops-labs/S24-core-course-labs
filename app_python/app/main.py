...
app.config['datetime_format'] = os.getenv('DATETIME_FORMAT', default='%Y-%m-%d %H:%M:%S %z')


@app.route("/api/v1/time")
def get_time():
    """
    method to fetch current time in a configured timezone in a configured format

    :return: dictionary containing "time" field or an "error" filed
    """
    try:
        app_timezone = pytz.timezone(app.config['timezone'])
        time = app_timezone.localize(datetime.now(), is_dst=None)
        formatted_time = time.strftime(app.config['datetime_format'])
