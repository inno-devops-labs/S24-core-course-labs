...
This module contains unit tests for the functions and routes implemented in the Flask web app that displays the current
 time in Moscow. The tests are designed to verify the correctness and functionality of the app's core features.
...
Note:
    - These tests assume that the Flask app is running correctly and the HTML template is properly set up.
    - It's recommended to run the tests in a controlled environment, as they may interact with external dependencies
     such as the current time and Flask routes.
...
class TestApp(unittest.TestCase):
    ...
    This class contains individual test methods to validate specific functionalities of the Flask web app.
    """
    ...
    def setUp(self):
        ...
        Initializes resources required for testing, such as the Flask app client.
        """
    ...
    def test_time_accuracy(self):
        ...
        Verify the accuracy of the time retrieved by the get_moscow_time function against the actual time in
        ...
        self.assertAlmostEqual(time.timestamp(), correct_time.timestamp(), 0,
                               'The received time is not synchronized with the Europe/Moscow timezone.')

    def test_time_format(self):
        """
        Test Time Format

        Validate the format of the time string returned by the formatted_time function.
        """
        ...
        self.assertTrue(correct_format, 'The received time format is not appropriate')
    ...
    def test_display_moscow_time(self):
        ...
        self.assertEqual(response.status_code, 200, 'The received status code is not 200')
