...
def test_display_time_route_current_time(self):
    ...
    self.assertIsNotNone(current_time_tag,
                         "Could not find any <p> tag in the HTML response.")
    ...
    expected_time = (datetime.now(pytz.timezone('Europe/Moscow'))
                     .replace(tzinfo=None))

    self.assertAlmostEqual(current_time, expected_time,
