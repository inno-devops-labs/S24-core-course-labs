...
## Helm Chart Hooks
I added `pre-install-hook.yaml` and `post-install-hook.yaml` for both apps under `templates`. Also, I added a library chart under `/libchart`, added it as a dependency for both applications, and ran `helm dependency update`.
