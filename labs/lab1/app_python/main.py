...
class Router:
    ...
    def display_date_time() -> str:
        ...
        try:
            ...
            now: datetime = utc_now.replace(tzinfo=pytz.utc).astimezone(moscow_tz)

            date_time: DateTimeFormatted = DateTimeFormatted.from_datetime(now)

            return render_template(
                "index.html", current_date=date_time.date, current_time=date_time.time
            )
        except Exception as e:
            logging.error(
                f"Got error while handling request. Error: {e}", exc_info=True
            )
            return render_template("index.html", current_date="N/A", current_time="N/A")
