...
class TestGlobalMoscowTime:
    ...
    def test_global_moscow_time_time_format(self, response):
        ...
        try:
            datetime.strptime(
                response_json["moscow_time"], "%Y-%m-%dT%H:%M:%S.%f%z"
            )
        except ValueError:
    ...
    def test_global_moscow_time_timezone(self, response):
        ...
        assert get_utc_offset(api_moscow_datetime) == get_utc_offset(
            datetime.now(pytz.timezone("Europe/Moscow"))
        ), "Wrong response timezone!"

    def test_global_moscow_time_time(self, response):
        """
        Testing the correspondence of the api time to the NTP server
        Moscow time.
        """
        response_json = response.json()
        api_moscow_time = string_to_datetime(
            response_json["moscow_time"]
        ).timestamp()
        current_moscow_time = float(
            ntplib.NTPClient().request("pool.ntp.org").tx_time
        )
        assert api_moscow_time == pytest.approx(
            current_moscow_time, 1.0e-3
        ), "Wrong response time!"
